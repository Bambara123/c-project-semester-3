...
int main(){
    ...
    word_space *array = malloc(sizeof(word_space)*100); // horizontal spaces array.
    word_space *array_v = malloc(sizeof(word_space) * 100); // vertical spaces array.
    word_space *combined_array = malloc(sizeof(word_space)* 100); // combined spaces array (vertical array + horizontal array = combined array)
    ...
    char *temp = (char*)malloc(sizeof(char)); // To capture the new line.

    int *impossible = (int*)malloc(sizeof(int)); // To indicate can be solved or not.
    *impossible = 0;
    ...
    int *tempc = (int*)malloc(sizeof(int)); // Check grid for invalid inputs
    
    *tempc=check_grid(puzzle_array); 
...
// ---------------------------------------------------------------------------------------------------------------------------------------
    ...
    *word_space_array_count = 0; 

    for(int x = 0; x < puzzle_row; x++){

        int word_length = 0;
        int start_row = 0, start_colo = 0; 
...
// -------------------------------------------------------------------------------------------------------------------- 
    ...
    *word_space_array_count_v = 0; 
    ...
    for(int x = 0; x < puzzle_colo; x++){
        ...
        int word_length_v = 0;
        int start_row_v = 0, start_colo_v = 0;
    ...
    int *c_a_count = (int*)malloc(sizeof(int));
    *c_a_count = 0; // combine horizontal and vertical arrays.
    ...
    *c_a_count = *word_space_array_count + *word_space_array_count_v; // number of elements in combined array
    ...
    free(word_space_array_count); // Free the variables allocated.
...
// -------------------------------------------------------------------------------------------------------------------------------
    ...
    free(c_a_count); // free combined array counting variable.
    free(array);
    free(array_v);
    free(combined_array);
    free(used_word_bool);
...
int check_grid(char **puzzle_array){   //function to check  wether the given puzzle is valid or not
    ...
    for(int i=0;i<puzzle_row;i++){
        for(int j=0;j<No_of_Columns(puzzle_array);j++){
            if(puzzle_array[i][j]!='*' && puzzle_array[i][j]!='#' && isalpha(puzzle_array[i][j])==0 ){//check if there is symbol or number in the grid
                c_=1;
                break;
            }
        }
