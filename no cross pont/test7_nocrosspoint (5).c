...
// -------------------------------------------------------------------------------------------------------------------------------- // filling spaces.
    ...
    for(int x = 0; x < c_a_count; x++){
        ...
        int words_tested = 0, no_word_for_length_count = 0, no_word_for_letters = 0, have_words_for_length_count = 0;


        for(int y = 0; y < 4; y++){ // 3 = length of the word list.
            ...
            printf("word filled index = %d, word filled count - %d the word tested - %s\n",word_filled_ind, combined_array[x].word_fill_count, word_list[y]);

            if(word_filled_ind == combined_array[x].word_fill_count){

                words_tested++; // Increase the amount of words tested. Excluding word already added to space array.
                ...
                if(strlen(word_list[y]) == combined_array[x].length  && used_word_bool[y] == 0){
                    ...
                    if(combined_array[x].direction == 'h'){ // fill horizontally.
                        ...
                        int non_hash_count = 0;// start here
                        ...
                        }else{ //  This executes, if the word cannot satisfy letters for already filled ones.
                    ...
                    if(combined_array[x].direction == 'v'){// fill vertically.
                        ...
                        if(letter_count == non_hash_count){ //   This means every non hash index have similar letters.
                            ...
                            strcpy(combined_array[x].words_filled[combined_array[x].word_fill_count], word_list[y]);
                            printf("word filled in space arrray - %s", combined_array[x].words_filled[combined_array[x].word_fill_count]);
                            strcpy(used_words[x], word_list[y]); // add to used words list.
                ...
                }else{ // else no word with same amount of letters. - go back

                    no_word_for_length_count++; // better to check another word from the list.

                }

            }else{ // If all possible words can be used to to fill the space are in the words_filled array of the space. no word can choose,  go to last space with same length.

                printf("no word found - need to go next loop - reason - every word can be tried in the space array.\n");

                int combined_array_index = x;
                ...
                strcpy(final_word,combined_array[combined_array_index].words_filled[combined_array[combined_array_index].word_fill_count]);

                for(int used_word_list_index = x; used_word_list_index >= combined_array_index; used_word_list_index--){ // make all used word bools 0 till previous point.
                ...
                for(int index = filled_index_index ; (index_array[index].x != combined_array[combined_array_index].colo && index_array[index].y != combined_array[x].colo) ; index--){
        ...
        if(words_tested == no_word_for_length_count){

            printf("NO words for this length.\n");
        ...
        }else if(no_word_for_letters == have_words_for_length_count){

            printf("space is - %d length one\n", combined_array[x].length);

            printf("You have words for length count, but we cannot found words to match with letters already filled.\n");
            ...
            printf("combined array index: %d\n", combined_array_index);
            ...
            printf("filled index index: %d\n", filled_index_index);

            for(int kk = 0; kk < filled_index_index; kk++){

                printf("row - %d colo - %d , %c \n", index_array[kk].x, index_array[kk].y, puzzle_array[index_array[kk].x][index_array[kk].y]);
            }
            int index;
